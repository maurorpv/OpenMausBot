...
describe("formatTokenCount", () => {
  ...
  it("formats counts 10,000 and above using compact notation", () => {
    expect(formatTokenCount(10000)).toBe("~10k tok");
    expect(formatTokenCount(32400)).toBe("~32.4k tok");
...
describe("large attachment regression", () => {
  it("bounds estimation cost for large payloads without hanging", () => {
    const line = "const result = calculateMetrics(item, index, options);\n";
    const largePayload = line.repeat(10_000); // ~550 KB payload
    const start = performance.now();
    const metrics = getTextMetrics(largePayload);
    const duration = performance.now() - start;

    expect(metrics.words).toBeGreaterThan(10_000);
    expect(metrics.characters).toBe(largePayload.length);
    expect(metrics.estimatedTokens).toBeGreaterThan(10_000);
